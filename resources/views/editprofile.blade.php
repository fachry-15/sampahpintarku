    ...
    <div class="bg-white rounded-xl shadow-xs p-2 dark:bg-neutral-800">
      ...
      <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data">
        ...
        <div class="flex items-center gap-5">
          <img class="inline-block size-16 rounded-full ring-2 ring-white dark:ring-neutral-900" src="{{ $user->profile_photo_url ?? 'https://preline.co/assets/img/160x160/img1.jpg' }}" alt="Avatar">
        ...
        <label for="af-account-email" class="inline-block text-sm text-gray-800 mt-2.5 dark:text-neutral-200">
          ...
          <div class="sm:col-span-9">
        <input id="af-account-email" type="email" name="email" class="py-1.5 sm:py-2 px-3 pe-11 block w-full border-gray-200 shadow-2xs sm:text-sm rounded-lg focus:border-blue-500 focus:ring-blue-500 checked:border-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600" placeholder="masukkan alamat email" value="{{ old('email', $user->email) }}">
        @error('email')
          <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
        @enderror
          </div>
        ...
        <div class="inline-block">
          ...
          <div class="sm:col-span-9">
        <div class="sm:flex">
          <input id="af-account-phone" name="phone_number" type="text" class="py-1.5 sm:py-2 px-3 pe-11 block w-full border-gray-200 shadow-2xs -mt-px -ms-px first:rounded-t-lg last:rounded-b-lg sm:first:rounded-s-lg sm:mt-0 sm:first:ms-0 sm:first:rounded-se-none sm:last:rounded-es-none sm:last:rounded-e-lg sm:text-sm relative focus:z-10 focus:border-blue-500 focus:ring-blue-500 checked:border-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600" placeholder="contoh: 6289XXXXXXXX" value="{{ old('phone_number', $user->phone_number) }}">
        </div>
        <small class="text-gray-400 dark:text-neutral-400">
          * Pastikan no telepon aktif karena akan digunakan sebagai media komunikasi dengan pihak tempat sampahku dan gunakan awalan 62.
        </small>
        @error('phone_number')
          <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
        @enderror
