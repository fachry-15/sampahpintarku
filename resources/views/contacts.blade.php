<x-app-layout>
    <div class="w-full lg:ps-64 ">
        ...
        <div class="text-left py-4">
            <h1 class="text-6xl font-extrabold text-gray-800 dark:text-neutral-200">Daftar pesan pengguna</h1>
            <p class="mt-2 text-sm font-normal text-gray-500 dark:text-neutral-400">Berikut adalah laporan pesan, kritik, saran, dan informasi dari pengguna</p>
        </div>
              ...
              <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
                ...
                <tr>
                  <td colspan="7" class="text-center py-4">
                    <span class="text-sm text-gray-500 dark:text-neutral-500">Mohon maaf, belum ada pesan atau kritik yang diberikan.</span>
                  </td>
                </tr>
                @else
                  ...
                  <tr>
                    ...
                    <td class="h-px w-72 whitespace-nowrap">
                        <div class="px-6 py-3">
                            <span class="block text-sm font-semibold text-gray-800 dark:text-neutral-200">{{ $data->phone }}</span>
                        </div>
                      </td>
                      <td class="h-px w-72 whitespace-nowrap">
                        <div class="px-6 py-3">
                            <span class="block text-sm font-semibold text-gray-800 dark:text-neutral-200">{{ Str::substr($data->message, 0, 50) }}{{ strlen($data->message) > 50 ? '...' : '' }}</span>
                        </div>
                      </td>
                    <td class="size-px whitespace-nowrap">
                      <div class="px-6 py-3">
                    ...
                    <td class="size-px whitespace-nowrap text-center">
                      ...
                      <!-- Modal for Each Data -->
                      <div id="hs-medium-modal-{{ $data->id }}" class="hs-overlay hidden size-full fixed top-0 start-0 z-80 overflow-x-hidden overflow-y-auto pointer-events-none" role="dialog" tabindex="-1" aria-labelledby="hs-medium-modal-label-{{ $data->id }}">
                        <div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all md:max-w-2xl md:w-full m-3 md:mx-auto">
                          <div class="flex flex-col bg-white border border-gray-200 shadow-2xs rounded-xl pointer-events-auto dark:bg-neutral-800 dark:border-neutral-700 dark:shadow-neutral-700/70">
                            <div class="flex justify-between items-center py-3 px-4 border-b border-gray-200 dark:border-neutral-700">
                              <h3 id="hs-medium-modal-label-{{ $data->id }}" class="font-bold text-gray-800 dark:text-white">
                                Contact Details
                              </h3>
                              <button type="button" class="size-8 inline-flex justify-center items-center gap-x-2 rounded-full border border-transparent bg-gray-100 text-gray-800 hover:bg-gray-200 focus:outline-hidden focus:bg-gray-200 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-700 dark:hover:bg-neutral-600 dark:text-neutral-400 dark:focus:bg-neutral-600" aria-label="Close" data-hs-overlay="#hs-medium-modal-{{ $data->id }}">
                            ...
                            <div class="p-4 overflow-y-auto">
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6 p-4 bg-gray-50 dark:bg-neutral-800 rounded-lg shadow-md">
                                  <div>
                                  <span class="block text-sm font-medium text-gray-500 dark:text-neutral-400">Nama Pengirim</span>
                                  <p class="text-lg font-semibold text-gray-800 dark:text-white">{{ $data->name }}</p>
                                  </div>
                                  <div>
                                  <span class="block text-sm font-medium text-gray-500 dark:text-neutral-400">Email</span>
                                  <p class="text-lg font-semibold text-gray-800 dark:text-white">{{ $data->email }}</p>
                                  </div>
                                  <div>
                                  <span class="block text-sm font-medium text-gray-500 dark:text-neutral-400">No. Telpon</span>
                                  <p class="text-lg font-semibold text-gray-800 dark:text-white">{{ $data->phone }}</p>
                                  </div>
                                  <div>
                                  <span class="block text-sm font-medium text-gray-500 dark:text-neutral-400">Waktu pelaporan</span>
                                  <p class="text-lg font-semibold text-gray-800 dark:text-white">{{ $data->created_at->setTimezone('Asia/Jakarta')->format('d M Y, H:i') }}</p>
                                  </div>
                                </div>
                                <div class="p-4 bg-gray-50 dark:bg-neutral-800 rounded-lg shadow-md">
                                  <span class="block text-sm font-medium text-gray-500 dark:text-neutral-400">Pesan</span>
                                  <p class="text-base text-gray-800 dark:text-white leading-relaxed border-l-4 border-blue-500 pl-4">{{ $data->message }}</p>
                                </div>
                            </div>
                            <div class="flex justify-end items-center gap-x-2 py-3 px-4 border-t border-gray-200 dark:border-neutral-700">
                              <button type="button" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-2xs hover:bg-gray-50 focus:outline-hidden focus:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-800 dark:border-neutral-700 dark:text-white dark:hover:bg-neutral-700 dark:focus:bg-neutral-700" data-hs-overlay="#hs-medium-modal-{{ $data->id }}">
                                Close
                              </button>
                            </div>
                          </div>
