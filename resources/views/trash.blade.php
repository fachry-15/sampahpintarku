    ...
    <div class="w-full lg:ps-64 ">
        ...
        <ol class="flex items-center whitespace-nowrap">
            ...
            <li class="inline-flex items-center text-sm font-semibold text-gray-800 truncate dark:text-neutral-200" aria-current="page">
              Daftar Manajemen Pengambilan Sampah
            </li>
          </ol>
        <div class="text-left py-4">
            <h1 class="text-6xl font-extrabold text-gray-800 dark:text-neutral-200">Daftar Manajemen Pengambilan Sampah</h1>
            <p class="mt-2 text-sm font-normal text-gray-500 dark:text-neutral-400">Berikut adalah halaman manajemen pengambilan sampah oleg petugas</p>
        </div>
          ...
          <div class="bg-white border border-gray-200 rounded-xl shadow-2xs overflow-hidden dark:bg-neutral-900 dark:border-neutral-700">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
              ...
              <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
                @if ($jadwal_pengambilans->isEmpty())
                <tr>
                  <td colspan="3" class="text-center py-4">
                    <span class="text-sm text-gray-500 dark:text-neutral-500">Mohon maaf, belum ada penjadwalan pengambilan yang tersedia saat ini.</span>
                  </td>
                </tr>
                @else
                  @foreach ($jadwal_pengambilans as $data)
                  <tr>
                    ...
                    <td class="h-px w-72 whitespace-nowrap">
                      <div class="px-6 py-3">
                          <span class="block text-sm font-semibold text-gray-800 dark:text-neutral-200">
                              {{ \Carbon\Carbon::parse($data->waktu_pengambilan)->addHours(7)->locale('id')->translatedFormat('D, d M Y H:i') }}
                          </span>
                      </div>
                    </td>
                    <td class="h-px w-72 whitespace-nowrap">
                      <div class="px-6 py-3">
                          <span class="block text-sm font-semibold text-gray-800 dark:text-neutral-200">
                             {{ \Carbon\Carbon::parse($data->created_at)->addHours(7)->locale('id')->translatedFormat('D, d M Y H:i') }}
                          </span>
                      </div>
                    </td>
                    <td class="h-px w-72 whitespace-nowrap">
                    ...
                    @else
                        <span class="py-1 px-1.5 inline-flex items-center gap-x-1 text-xs font-medium bg-green-100 text-green-800 rounded-full dark:bg-green-500/10 dark:text-green-500">
                            <svg class="size-2.5" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                                <path d="M16 8a8 8 0 1 1-16 0 8 8 0 0 1 16 0zM7.293 10.707a1 1 0 0 0 1.414 0l3-3a1 1 0 0 0-1.414-1.414L8 8.586 6.707 7.293a1 1 0 0 0-1.414 1.414l2 2z" />
                            </svg>
                            Disetujui
                        </span>
                    @endif
                    ...
                    <td class="size-px whitespace-nowrap">
                        <form action="{{ route('sampah.update', ['id' => $data->id]) }}" method="POST" class="inline">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="py-2 px-4 my-2 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-teal-500 text-white hover:bg-teal-600 focus:outline-hidden focus:bg-teal-600 disabled:opacity-50 disabled:pointer-events-none">
                                Setujui
                            </button>
                        </form>
                    </td>
